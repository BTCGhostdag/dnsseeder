...
[![GoDoc](https://img.shields.io/badge/godoc-reference-blue.svg)](http://godoc.org/github.com/BTCGhostdag/dnsseeder)
...
`BTCD` node specified with the `-s` flag and listen for `addr`
...
file, so on subsequent start ups the `BTCD` node specified with `-s`
...
* Launch a `BTCD` node for the DNSSeeder to connect to
...
```bash
git clone https://github.com/BTCGhostdag/dnsseeder ${GOPATH}/src/github.com/BTCGhostdag/dnsseeder
cd ${GOPATH}/src/github.com/BTCGhostdag/dnsseeder
go install .
```
...
`BTCD` instances, set the following DNS records:
